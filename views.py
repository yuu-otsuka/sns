...
from .forms import GroupCheckForm, GroupSelectForm, SearchForm, \
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['mode'] == '__check_form__':
            ...
            # チェックされたGroup名をリストに纏める
            glist = []
            for item in request.POST.getlist('gruops'):
                glist.append(item)
            # Messageの取得
        ...
        if request.POST['mode'] == '__search_form__':
            ...
            checkform = GroupCheckForm(request.POST)
            # Groupのリストを取得
...
def groups(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['mode'] == '__friends_form__':
            ...
            # Userのリストに含まれるユーザーが登場したFriendを取得
            fds = Friend.objects.filter(owner=request.user)\
...
def get_your_group_message(owner, glist, find):
    ...
    # groupがgroupsに含まれか、me_groupsに含まれるMessageの取得
    if find is None:
